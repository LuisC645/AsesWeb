...
db.collection("deportistas_atl").get().then((querySnapshot) => {
...
db.collection("competencias_atl").get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
...
document.getElementById("formulario").addEventListener("submit", (event) => {
    ...
    db.collection("deportistas_atl").doc(documento).get()
    .then((doc) => {
        if (doc.exists) {
            ...
            db.collection("competencias_atl").doc(competencia).collection(documento).get()
            .then((querySnapshot) => {
...
document.getElementById("formulario_distancia").addEventListener("submit", (event) => {
    ...
    db.collection("competencias_atl").doc(competencia).get()
        .then(doc => {
            fecha = doc.data().fecha;
        })

    db.collection("competencias_atl").doc(competencia).collection(documento).doc(distancia)
        .get().then(comp => {
        if (comp.exists){
